...
Usage: python2 file.py <name> <Master_IP> <Master_Port> <SCTP_Port> <IP> <Port>
...
def generate_files(input_list):
    ...
    Output: A running fileserver
    """

    Name = input_list[1]
    Master_IP = input_list[2]
    Master_Port = input_list[3]
    SCTP_Port = input_list[4]
    IP = input_list[5]
    Port = input_list[6]
    ...
    "{phoenix,[{stacktrace_depth,20}]}," + "\n" +
                     "{connect,[{master,false},{master_info, {\"%s\",\"%d\"}}, {sctp_port, %d}, {default_port,\"%d\"}]}]." % ( Master_IP,int(Master_Port), int(SCTP_Port), int(Port)))
    configfile.close()
...
def main(input_list):
    ...
    if len(input_list) == 2:
        sctpfile = open('./releases/0.1.0/sock.cfg', 'r')
        sctp = sctpfile.readline().rstrip('/n')
        os.system("echo -n '%s' | xclip -selection clipboard" % sctp)
        generate_files([input_list[0], "file%s" % (input_list[1]), "127.0.0.1", "4010", sctp, "127.0.0.1", "401%s" % (input_list[1])])
        ...
        try:
            ...
            print e
            print >> sys.stderr, "Usage: python2 file.py <name> <Master_IP> <Master_Port> <SCTP_Port> <IP> <Port>"
